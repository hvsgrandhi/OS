...
int main()
{
    ...
    pid_t pid = fork(); // To fork the current process to create child process. The parent and child will communicate through pipes if (pid == -1)
    {
        perror("Fork failed");
        exit(EXIT_FAILURE);
    }
    if (pid == 0)
    {
        ...
        while (1)
        {
            ...
            ssize_t bytes_read = read(pipe1[READ_END], buffer, sizeof(buffer)); // Read data from 'pipe1' into 'buffer' and get no.of bytes read if (bytes_read <= 0) // Checking if end of input has been reached
            {
                break; // End of input
            }
            for (ssize_t i = 0; i < bytes_read; i++) // Loop through 'buffer' to count characters, words and lines
                {
                    charCount++;
